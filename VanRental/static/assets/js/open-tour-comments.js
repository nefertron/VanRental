...
function openAllTourComments(id){
    console.log(id)
    ...
    var open_btn = document.getElementById(`open-tour-comments_${id}`)
    open_btn.style.display = 'none';
    
    var hide_btn = document.getElementById(`hide-open-tour-comments_${id}`)
    hide_btn.style.display = '';
}

function hideAllTourComments(id){
    var all_user_comments = document.querySelectorAll(`[id^="${id}"]`);
    for (var x = 0; x < all_user_comments.length; x++){
        if (x < 1){
            all_user_comments[x].style.display = '';
