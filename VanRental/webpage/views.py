...
def list_of_vans(request):
    ...
    if request.method == "POST":
        ...
        if Van.objects.filter(plate_no = plate_no).first():
            ...
            if not images is None and len(images.split('|||')) > 0:
                image_list = images.split('|||') # use to split the image addresses generated by the javascript
                plate_no = RemoveSpaces(plate_no)

                days_of_the_week = ['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday', 'Sunday'] # will be used to get the color coding
                color_coding = '' #initial color coding value
                for i, day in enumerate(days_of_the_week): # loop the array as i will be the index and day will be the value
                    code = request.POST.get(f'{day}')  # each loop get the value of each checkbox
                    if code == 'on': 
                        color_coding += day
                        if i < len(days_of_the_week) - 1:
                            color_coding += '|||' # separate each value with |||


                trueOrFalse = is_airconditioned == 'yes'
                ...
                # update the van information

                to_edit_van_id = request.POST.get('to_edit_van_id')
                if to_edit_van_id:
                    days_of_the_week = ['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday', 'Sunday'] # will be used to get the color coding
                    color_coding = '' #initial color coding value
                    for i, day in enumerate(days_of_the_week): # loop the array as i will be the index and day will be the value
                        code = request.POST.get(f'to_edit_{day}')  # each loop get the value of each checkbox
                        if code == 'on': 
                            color_coding += day
                            if i < len(days_of_the_week) - 1:
                                color_coding += '|||' # separate each value with |||

                    is_airconditioned = request.POST.get('to_edit_is_airconditioned'),
                    trueOrFalse = is_airconditioned == 'yes'

                    Van.objects.filter(id = to_edit_van_id).update(
                        plate_no = request.POST.get('to_edit_plate_no'),
                        brand_name = request.POST.get('to_edit_brand_name'),
                        color = request.POST.get('to_edit_color'),
                        is_airconditioned = trueOrFalse,
                        description = request.POST.get('to_edit_description'),
                        color_coding = color_coding
                    )


                    message = f'You successfully update the van information with plate no. {request.POST.get("to_edit_plate_no")}'
                    messages.info(request, message)
                    return redirect('/list-of-vans')
...
def get_unavailable_dates(request, id):
    ...
    all_color_coding = []
    van = Van.objects.filter(id = id).first()
    for x in van.color_coding.split('|||'):
        print('x ', x)
        if x == 'Monday':
            all_color_coding.append(0)
        elif x == 'Tuesday':
            all_color_coding.append(1)
        elif x == 'Wednesday':
            all_color_coding.append(2)
        elif x == 'Thursday':
            all_color_coding.append(3)
        elif x == 'Friday':
            all_color_coding.append(4)
        elif x == 'Saturday':
            all_color_coding.append(5)
        elif x == 'Sunday':
            all_color_coding.append(6)
    ...
    while date_today <= end_date:
        ...
        if date_today.weekday() in all_color_coding:  # 5 is Saturday, 6 is Sunday
            # Format the date as "YYYY-MM-DD" string and append to the list
            formatted_date = date_today.strftime('%Y-%m-%d')
            if not formatted_date in temp_storage:
                temp_storage.append(formatted_date) 
...
def get_van_info(request, id):
    van = Van.objects.filter(id = id).first()

    all_info  = {
        'brand_name' : van.brand_name,
        'plate_no' : van.plate_no,
        'color' : van.color,
        'number_of_seats' : van.number_of_seats,
        'description' : van.description,
        'is_airconditioned' : van.is_airconditioned,
        'color_coding' : van.color_coding.split('|||')
    }
